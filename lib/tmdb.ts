...
async function tmdbFetch(path: string, searchParams?: Record<string, string>) {
  ...
  if (!response.ok) {
    console.error("TMDB request failed", response.status, path, await response.text());
...
function imdbUrl(id?: string | null) {
  return id ? `https://www.imdb.com/title/${id}/` : null;
}

async function externalImdbId(tmdbId: number) {
  const payload = (await tmdbFetch(`/movie/${tmdbId}/external_ids`)) as {
    imdb_id?: string | null;
  };
...
async function toSuggestion(
  ...
  const id = knownImdbId ?? (await externalImdbId(movie.id).catch(() => null));
...
export async function searchTmdbMovies(
  query: string,
  options?: { limit?: number; matchReason?: string },
): Promise<MovieSuggestion[]> {
  ...
  return Promise.all(
    (payload.results ?? [])
      ...
      .map(movie => toSuggestion(movie, options?.matchReason)),
  );
}
...
export async function findTmdbMovieByImdbId(
  ...
  const payload = (await tmdbFetch(`/find/${encodeURIComponent(imdbId)}`, {
...
export async function getTmdbMovieById(
  tmdbId: number,
  matchReason = "Direkte treff via TMDB",
  ...
  return toSuggestion(movie, matchReason);
}
