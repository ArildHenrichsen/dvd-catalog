...
export default async function EditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <article className="detail edit-release-page">
      <Link className="back-link" href={`/releases/${release.id}`}>
        ← Tilbake til {release.original_title}
      </Link>
      <header className="edit-release-heading">
        <span className="eyebrow">Rediger DVD</span>
        <h1>{release.original_title}</h1>
        {release.alternative_title && <p className="muted">{release.alternative_title}</p>}
      </header>
